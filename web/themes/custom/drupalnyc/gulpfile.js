...
async function clean(cb) {
  ...
  try {
    ...
    console.error(`Error while deleting ${distPath} directory: ${error}`);
  }
...
function baseStylesWatch(cb) {
  gulp.src(paths.base.styles.src, {
      sourcemaps: true,
      since: gulp.lastRun(baseStylesWatch),
    })
...
function baseScriptsWatch(cb) {
  gulp.src(paths.base.scripts.src, {
      since: gulp.lastRun(baseScriptsWatch),
    })
...
exports.watch = () => {
  ...
  browserSync.init({
    proxy: process.env.BS_PROXY || "http://drupalnyc.lndo.site",
